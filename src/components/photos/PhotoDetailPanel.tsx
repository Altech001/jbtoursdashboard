...
const PhotoDetailPanel: React.FC<PhotoDetailPanelProps> = ({ photo, onClose, onLike, onDelete }) => {
  ...
  return (
    <div className="fixed inset-0 bg-opacity-50 z-40" onClick={onClose}>
      ...
      <button
        className="w-full flex items-center justify-center gap-2 rounded-lg bg-red-100 text-red-700 px-4 py-2.5 hover:bg-red-200 dark:bg-red-900/50 dark:text-red-300 dark:hover:bg-red-900"
        onClick={() => photo && onDelete(photo.id)}
      >
        <Trash2 size={18} />
        Delete
      </button>
