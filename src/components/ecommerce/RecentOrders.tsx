...
// Interface for a booking record from the API
interface Booking {
  id: string;
  name: string;
  email: string;
...
export default function RecentOrders({ bookings }: RecentOrdersProps) {
  const getStatus = (checkinDate: string): "Delivered" | "Pending" => {
    ...
    dateOfCheckin.setHours(0, 0, 0, 0);

    return dateOfCheckin < today ? "Delivered" : "Pending";
  };
        ...
        <Table>
          ...
          <TableBody className="divide-y divide-gray-100 dark:divide-gray-800">
            {bookings.map((booking) => {
              const status = getStatus(booking.checkin_date);
              return (
