...
import { Edit2, MoreVertical, ScanEye, Trash2 } from "lucide-react";
import { useEffect, useState } from "react";
import { ClipLoader } from "react-spinners";
import PageBreadCrumb from "../../components/common/PageBreadCrumb";
import InputField from "../../components/form/input/InputField";
    ...
    return (
        <div className="relative">
            ...
            >
                ...
                <DropdownItem onItemClick={() => { onEdit(booking); onToggle(); }} className="flex items-center gap-2">
                    <Edit2 className="size-4" /> Edit
                </DropdownItem>
                <DropdownItem onItemClick={() => { onDelete(booking.id); onToggle(); }} className="flex items-center gap-2 text-red-500 hover:bg-red-50 dark:hover:bg-red-500/10">
...
const BookForms = () => {
  ...
  useEffect(() => {
    const results = bookings.filter((booking: any) =>
      booking.name.toLowerCase().includes(searchTerm.toLowerCase())
    );
    setFilteredBookings(results);
  }, [searchTerm, bookings.length]); // Fixed: using bookings.length instead of bookings
    ...
    <>
                  ...
                  <TableRow key={booking.id}>
                    ...
                    <TableCell className="px-4 py-3">
                      <ActionsDropdown 
                        ...
                        isLast={index === filteredBookings.length - 1}
                        onView={handleView} 
                        onEdit={handleEdit} 
      ...
      {isViewPanelOpen && selectedBooking && (
        <div className="fixed z-50 top-0 right-0 h-full w-2/5 bg-white p-8 shadow-lg dark:bg-boxdark overflow-y-auto">
          ...
          <div className="printable-receipt bg-gray-50 dark:bg-gray-800 p-6 rounded-lg font-mono">
            <div className="text-center mb-6">
              <h3 className="text-xl font-semibold">JB Heartfelt Tours and Travels</h3>
              <p className="text-sm text-gray-500">Booking Confirmation</p>
            </div>
            <div className="space-y-2 text-sm">
              <div className="flex justify-between"><span>Booking ID:</span><span>{selectedBooking.id}</span></div>
            ...
            <div className="space-y-2 text-sm">
              <h4 className="font-semibold">Additional Information</h4>
              <p><span className="font-semibold">Special Requests:</span> {selectedBooking.special_requests}</p>
              <p><span className="font-semibold">Activities:</span> {selectedBooking.activites}</p>
              <p><span className="font-semibold">Message:</span> {selectedBooking.message}</p>
            </div>
            <div className="text-center mt-6 text-xs text-gray-500">
              <p>Thank you for booking with us!</p>
      ...
      {/* Edit Panel */}
      {isEditPanelOpen && selectedBooking && (
        <div className="fixed z-50 top-0 right-0 h-full md:w-2/5 bg-white p-8 shadow-lg dark:bg-boxdark overflow-y-auto">
          <div className="flex justify-between items-center mb-6">
          ...
          <form>
            <div className="space-y-4">
              ...
              <div><Label htmlFor="destination">Destination</Label><InputField id="destination" name="destination" value={selectedBooking.destination} onChange={(e) => setSelectedBooking({ ...selectedBooking, destination: e.target.value })}/></div>
              <div><Label htmlFor="guest_capcity">Guest Capacity</Label><InputField id="guest_capcity" name="guest_capcity" type="number" value={selectedBooking.guest_capcity} onChange={(e) => setSelectedBooking({ ...selectedBooking, guest_capcity: Number(e.target.value) })}/></div>
              <div><Label htmlFor="checkin_date">Check-in Date</Label><InputField id="checkin_date" name="checkin_date" type="date" value={selectedBooking.checkin_date.split("T")[0]} onChange={(e) => setSelectedBooking({ ...selectedBooking, checkin_date: e.target.value })}/></div>
              <div><Label htmlFor="checkout_date">Check-out Date</Label><InputField id="checkout_date" name="checkout_date" type="date" value={selectedBooking.checkout_date.split("T")[0]} onChange={(e) => setSelectedBooking({ ...selectedBooking, checkout_date: e.target.value })}/></div>
              <div><Label>Special Requests</Label><TextArea rows={4} value={selectedBooking.special_requests} onChange={(value) => setSelectedBooking({ ...selectedBooking, special_requests: value })}/></div>
              <div><Label>Activities</Label><TextArea rows={4} value={selectedBooking.activites} onChange={(value) => setSelectedBooking({ ...selectedBooking, activites: value })}/></div>
